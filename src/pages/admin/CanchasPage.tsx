...
function CanchasPage() {
  ...
  async function guardarEdicion() {
    ...
    setGuardando(true);
    try {
      const actualizado = await actualizarCancha(editando.id, {
        ...form,
  ...
  async function toggleEstado(c: CanchaDto) {
    try {
      ...
      toast.success(
        actualizado.estado
          ? `Cancha ${actualizado.nombre} habilitada`
          : `Cancha ${actualizado.nombre} deshabilitada`,
      );
    } catch {
  ...
  const stats = [
    {
      ...
      value: `${activasCount} / ${canchas.length}`,
      delta: "En operación",
      color: "#71717a",
    },
    {
      ...
      <main className="flex flex-1 flex-col gap-6 p-7">
        ...
        <div className="grid grid-cols-4 gap-4">
          {stats.map((s) => (
            <div
              key={s.label}
              className="flex flex-col gap-2.5 rounded-[14px] border border-[#1f1f22] bg-[#0c0c0e] p-[18px]"
            >
              <span className="text-xs uppercase tracking-[.06em] text-[#71717a]">
                {s.label}
              </span>
        ...
        <div className="overflow-hidden rounded-[14px] border border-[#1f1f22] bg-[#0c0c0e]">
          <div className="flex items-center justify-between border-b border-[#1f1f22] px-5 py-4">
            ...
            <span className="text-[13px] text-[#71717a]">
              {canchasOrdenadas.length} canchas registradas
            </span>
          </div>
              ...
              >
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <span
                    ...
                    style={{ color: c.estado ? "#7fd970" : "#71717a" }}
                  >
          ...
          <div className="w-full max-w-sm rounded-[14px] border border-[#1f1f22] bg-[#0c0c0e] p-6">
            ...
            <div className="flex flex-col gap-4">
              ...
              <CampoNumero
                ...
                onChange={(v) => setForm({ ...form, precioHora: v })}
              />
              <CampoNumero
                label="Cantidad de jugadores"
                value={form.cantidadJugadores}
                onChange={(v) => setForm({ ...form, cantidadJugadores: v })}
              />
            </div>
          ...
          <div className="w-full max-w-sm rounded-[14px] border border-[#1f1f22] bg-[#0c0c0e] p-6">
            ...
            <div className="flex flex-col gap-4">
              <Campo
                ...
                onChange={(v) => setFormNueva({ ...formNueva, nombre: v })}
              />
              <Campo
                label="Descripción"
                value={formNueva.descripcion}
                onChange={(v) => setFormNueva({ ...formNueva, descripcion: v })}
              />
              <CampoNumero
                label="Precio por hora"
                value={formNueva.precioHora}
                onChange={(v) => setFormNueva({ ...formNueva, precioHora: v })}
              />
              <CampoNumero
                label="Cantidad de jugadores"
                value={formNueva.cantidadJugadores}
                onChange={(v) =>
                  setFormNueva({ ...formNueva, cantidadJugadores: v })
                }
              />
            </div>
            <div className="mt-6 flex justify-end gap-2">
